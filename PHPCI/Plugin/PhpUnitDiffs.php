...
//            $cmd = 'export SHELL_MODE_SCRIPT=phpunit && ' .' export IS_IN_CI=1 && ' . $phpunit . ' -d memory_limit=2000000M --process-isolation --tap %s -c "%s" ' . ' --filter="%s" ' . $this->coverage . $this->path ;
            $cmd = 'export APPLICATION_PLATFORM_ID=254 && export SHELL_MODE_SCRIPT=phpunit && ' .' export IS_IN_CI=1 && ' . $phpunit . ' -d memory_limit=4000000M --tap %s -c "%s" ' . ' --filter="%s" ' . $this->coverage . $this->path ;
