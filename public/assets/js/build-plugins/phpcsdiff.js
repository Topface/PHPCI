var phpcsdiffPlugin = PHPCI.UiPlugin.extend({
    id: 'build-phpcsdiff',
    ...
    register: function() {
        var self = this;
        var query = PHPCI.registerQuery('phpcsdiff-data', -1, {key: 'phpcsdiff-data'})
    ...
    render: function() {
        return $('<table class="table table-striped" id="phpcsdiff-data">' +
            '<thead>' +
            '<tr>' +
            '   <th>File</th>' +
            '   <th>Line</th>' +
...
PHPCI.registerPlugin(new phpcsdiffPlugin());
